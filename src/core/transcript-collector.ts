...
export class TranscriptCollector {
  ...
  async cleanupTranscripts(): Promise<{ invalidFormat: number; synthetic: number }> {
    ...
    try {
      ...
      if (invalidFormat > 0 || synthetic > 0) {
        logger.transcript.info(`Cleanup complete: removed ${invalidFormat} invalid format, ${synthetic} synthetic transcripts`);
      }

      return { invalidFormat, synthetic };
    } catch (error) {
